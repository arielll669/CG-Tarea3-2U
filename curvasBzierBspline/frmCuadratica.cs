...
namespace curvasBzierBspline
{
    public partial class frmCuadratica : Form
    {
        ...
        // Bitmap y Graphics para la capa estática (solo la curva verde ahora)
        ...
        private void DibujarElementosDinamicos()
        {
            ...
            // 3. Dibujar la capa de Rastro acumulado (si existe). ¡ESTA VA DEBAJO DE TODO LO DEMÁS!
            ...
            // IMPORTANTE: Dibujamos el rastro en la CAPA DE RASTRO (trailGraphics) para que se acumule,
            // pero el rastro es semitransparente para no tapar la curva verde.
            if (chkDejarRastro.Checked && isPlaying)
            {
                // Dibuja un pequeño punto del rastro en la capa de rastro
        ...
        private void PanelDibujo_MouseMove(object sender, MouseEventArgs e)
        {
            if (puntoArrastrado != null)
            {
                ...
                if (!throttledUpdate)
                {
                    // Al arrastrar, solo actualizamos la curva estática (la base verde)
                    DibujarCurvaEstatica();
                    DibujarElementosDinamicos();

                    throttledUpdate = true;
                    // Intentamos usar Task.Delay para resetear el flag (mejor fluidez en arrastre)
